...
function displayWeatherData(data) {
    ...
    let temperature = `${currentCondition.temp_C}°C`;
    let weather = `${currentCondition.weatherDesc[0].value}`;

    document.getElementById('weatherData').innerHTML = styleWeather(temperature, weather);
...
async function getWeather(location) {
    try {
        ...
        // Hint: Use document.getElementById() to select the element and update its innerHTML
        document.getElementById('weatherData').innerHTML = '<p class="bg-danger text-white rounded p-1">Error fetching and displaying weather data: ' + error + '</p>' 
    }
}
...
function styleWeather(temperature, weather) {
    let styledWeather = '<h2 class="bg-primary w-50 p-2 rounded text-white text-center">Temperature</h2><p class="text-center mb-3 w-50">' + temperature + '</p><h2 class="bg-primary w-50 p-2 rounded text-white text-center">Weather</h2><p class="text-center w-50">' + weather + '</p>'
